	...
	const t_camera *const camera, t_vector3d vector)
{
	...
	vector.x = -vector.y;
	vector.y = tmp;
	r_vector = rotate_vector_by_matrix(vector, camera->rotation_matrix);
...
t_color	calculate_color(const t_scene *const scene,
	const t_point3d vector_start, const t_vector3d vector,
	t_object3d *const *obj_ptr, const t_point3d *const point_ptr,
	const double *const dist_ptr, const double intensity,
	const int recursion_level)
{
	...
	if (material.Kd)
	{
		...
		{
			light_color = get_lighting_color(point, norm, scene);
	...
	if (material.Ks)
	{
		...
		if (scene->light_sources_len)
			specular_color = get_specular_color(point,
					reflected_ray, scene, material.p);
	}
	if (material.Kr)
	{
		...
		{
			reflected_color = trace_recursively(scene, point, reflected_ray,
					intensity * material.Kr, recursion_level + 1);
		}
		else
		{
	...
	result = rgb(0, 0, 0);
	if (material.Ka)
		result = add_colors(result, mul_color(ambient_color, material.Ka));
	if (material.Kd)
		result = add_colors(result, mul_color(diffuse_color, material.Kd));
	if (material.Ks)
		result = add_colors(result, mul_color(specular_color, material.Ks));
	if (material.Kr)
		result = add_colors(result, mul_color(reflected_color, material.Kr));
	return (result);
}

t_color	get_lighting_color(const t_point3d point,
	const t_vector3d norm_v, const t_scene *const scene)
{
	t_color				light_color;
	t_light_source3d	*ls;
	t_vector3d			v_ls;
...
t_color	get_specular_color(const t_point3d point,
	const t_vector3d reflected_ray, const t_scene *const scene, const double p)
{
	t_color				light_color;
	t_light_source3d	*ls;
	t_vector3d			v_ls;
...
t_bool	is_viewable(const t_point3d target_point,
	const t_point3d starting_point, const t_scene *const scene)
{
	...
	nearest_obj = NULL;
	nearest_intersection_point_dist = DBL_MAX;
	if (find_intersection_tree(scene->kd_tree, starting_point,
			ray, &nearest_obj, &nearest_intersection_point,
			&nearest_intersection_point_dist))
	{
