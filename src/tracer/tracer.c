	...
	const int recursion_level)
{
	...
	if (scene->fog_density)
	{
		result_color = add_colors(mul_color(scene->background_color,
			fog_density), mul_color(result_color, 1 - fog_density));
	}
...
inline t_color
get_lighting_color(const t_point3d point,
				const t_vector3d norm_v,
				const t_scene *const scene) {
...
inline t_color
get_specular_color(const t_point3d point,
				const t_vector3d reflected_ray,
				const t_scene *const scene,
				const double p) {
...
inline bool
is_viewable(const t_point3d target_point,
			const t_point3d starting_point,
			const t_scene *const scene) {
