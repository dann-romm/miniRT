...
t_color	get_color_sphere(const void *data,
	const t_point3d intersection_point)
{
	return (((t_sphere *)data)->color);
}

t_material	get_material_sphere(const void *data,
	const t_point3d intersection_point)
{
	return (((t_sphere *)data)->material);
}

t_vector3d	get_normal_vector_sphere(const void *data,
	const t_point3d intersection_point)
{
	t_vector3d	normal;
...
t_point3d	get_min_boundary_point_sphere(const void *data)
{
	...
	c = sphere->center;
	return (point3d(c.x - sphere->radius,
			c.y - sphere->radius, c.z - sphere->radius));
}
